...
PAR_DIR="${SCRIPT_DIR%/*}"
echo Parent directory is ${PAR_DIR}
...
echo The clustering radius has been set to ${CLUSTER_RADIUS}, and if you see this, the file "pipeline_params" was read successfully.
...
echo The primers are $PRIMER1 and $PRIMER2.
echo If seqtk worked, the reverse complement of the primers is: $PRIMER1RC and $PRIMER2RC
...
echo The folders you are about to analyze are as follows: $FOLDERLIST
# Define the number of directories to analyze
N_DIR=${#FOLDERLIST[@]}
echo That is a total of $N_DIR
...
echo The pipeline initiated successfully! Right now, it is working in $CURRENT_DIR
